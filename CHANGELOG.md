...
### Added
- **Incident enrichments from genai_incidents.** Entry→incident links now deep-link to the upstream incident page (`genai_incidents/incident/<id>.html`) instead of the raw JSON blob, and each incident now carries its upstream **MITRE ATLAS** (124/125) and **NIST AI RMF** (125/125) mappings — surfaced in a new "Framework Mappings" section on the incidents web app (ATLAS chips link to the hash-routed technique pages).
- **Incidents now feed from the `genai_incidents` source of truth.** Added `scripts/sync-incidents.mjs`, which pulls the curated incident tier from [emmanuelgjr/genai_incidents](https://github.com/emmanuelgjr/genai_incidents) and emits `data/incidents.json` as a generated view (owasp_llm+asi+dsgai → owasp_entries; MAESTRO/mitigations/references passthrough; upstream IDs kept). The Weekly Source Watch workflow now runs the sync + regenerate + validate and opens a PR, so the crosswalk stays current automatically. Incident set 114 (hand-curated) → **125** (synced curated tier); the incident schema was relaxed (5-digit IDs, optional MAESTRO/impact during upstream enrichment, `source` provenance block). `data/incidents.json` is no longer hand-edited — curate upstream.
- **STRIDE coverage gap closed** — added `agentic-top10/Agentic_STRIDE.md` (ASI01–ASI10) and `dsgai-2026/DSGAI_STRIDE.md` (DSGAI01–DSGAI21), mapping each risk to STRIDE categories (S/T/R/I/D/E) with tiered mitigations (STRIDE was previously LLM-only). Mapping files 68 → **70**; total mappings → **3,351**.
...
- **Evals overhaul (`evals/`)** — added 4 new Garak profiles (LLM06, plus DSGAI01/DSGAI12/DSGAI18 — first DSGAI coverage), a shared PyRIT `_harness.py` with LLM-as-judge scoring, version-pinned `evals/requirements.txt` (garak 0.15.0, PyRIT 0.13.0), an `evals/samples/` golden-reference set, and a `scripts/validate.js` evals check (valid OWASP ID + resolvable crosswalk path + no retired `data-security/` paths). Eval-profile total 25 → **29**.
- **Top-class eval tracks (`evals/`)** — added 5 new tool tracks so the suite spans the full eval matrix (offensive *and* defensive, runtime *and* static, model · app · agent · data-pipeline): `promptfoo/` (app/CI red-team, OWASP LLM+Agentic plugins), `inspect/` (Inspect AI native ASI01 task + AgentDojo/AgentHarm wiring mapped to the Agentic Top 10), `modelscan/` (ModelScan model-artifact supply-chain scan for LLM03/DSGAI17), `guardrails/` (Llama Prompt Guard 2 + NeMo Guardrails defense evaluation), and `privacy/` (Presidio PII output scan for DSGAI01 + canary extraction audit for DSGAI18). All version-pinned in `requirements.txt`; `validate.js` evals check extended to cover every track. Eval coverage now **8 tool tracks**.

### Fixed
- **Guardrail eval runtime-verified + made model-selectable.** Added a `PROMPT_GUARD_MODEL` env override and robust label handling (`LABEL_0/1`, `SAFE/INJECTION`, `benign/malicious`) to `evals/guardrails/prompt_guard_eval.py` so the track runs without the gated Meta model. Verified against the non-gated `protectai/deberta-v3-base-prompt-injection-v2` → 6/6 malicious detected, 0 false positives, PASS. **All 8 eval tracks are now runtime-verified** (Meta Prompt Guard 2 remains the gated default).
- **PII output scanner precision tuning** (`evals/privacy/pii_output_scan.py`): default now checks only high-precision identifiers (credit card, SSN, email, IBAN, IP, phone, passport, bank/medical numbers); the noisy NER entities (PERSON, LOCATION, NRP, DATE_TIME) that over-flagged benign text ("weather in Lisbon" → LOCATION) are opt-in via a new `--broad` flag. Re-verified locally.
- **Remaining eval tracks runtime-verified** vs `gpt-4o-mini`: Inspect (`agentic_goal_hijack`, accuracy 1.000), privacy (canary audit PASS; Presidio PII scan detects EMAIL/CREDIT_CARD), and NeMo Guardrails (input rail blocked a jailbreak). 7 of 8 tracks now runtime-verified; only Prompt Guard 2 is blocked by its gated HF model. No new bugs found in these.
- **Live eval verification fixed two bugs offline checks missed.** Running the tracks against `gpt-4o-mini` revealed: (1) PyRIT 0.13.0 `OpenAIChatTarget` appends `/chat/completions` to the endpoint, so `evals/pyrit/_harness.py` passing the full URL produced a doubled path (404) — now passes the base URL; (2) garak does not strip whitespace from a folded-YAML `probe_spec`, so every probe after the first became "unknown" — all 17 configs now use a single-line `probe_spec`. garak (`LLM01` end-to-end + all 17 configs load probes) and PyRIT (`llm01` end-to-end with LLM-judge scoring) are now **runtime-verified**; see `evals/README.md` → Verification status.
- **Normalised DSGAI heading wording across all 23 DSGAI files.** Headings and quick-reference name cells used inconsistent variants of the same risk (e.g. "Anonymization" vs "Anonymisation", "Data Governance & Lifecycle" vs "…Lifecycle & Classification", "…Exchange Risks", "…in AI Pipelines", en-dash vs em-dash). All now use one canonical set — 21 distinct headings, identical across files. Headings/quick-ref cells only; no mapping content changed.
- **Four DSGAI files were on a pre-release taxonomy.** `DSGAI_NHI`, `DSGAI_NISTSP80082`, and `DSGAI_SAMM` used the old "Prompt Injection via Data Channels…" taxonomy and `DSGAI_SP800218A` used a third ("Data Access Logging…", "Bias in Data…") — entries described different risks than the canonical DSGAI 2026 set, undetected because the validator only checked the ID token. All 21 entries in each file rewritten to the canonical taxonomy with correct framework mappings (SAMM v2.0 practices; OWASP NHI Top 10; NIST SP 800-82 Rev 3 / SP 800-53; NIST SP 800-218A SSDF). Also fixed the malformed `## Why` section and en-dash headings in `DSGAI_SP800218A`. Added a `scripts/validate.js` guard that rejects retired DSGAI taxonomy names so this cannot recur.
